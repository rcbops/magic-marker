...
coverage-html: ## check code coverage with an HTML report
	py.test --cov-report html --cov=magic_marker tests/
	$(BROWSER) htmlcov/index.html

coverage-term: ## check code coverage with a simple terminal report
	py.test --cov-report term-missing --cov=magic_marker tests/
...
install-editable: ## install the package in editable mode
	if pip list -e | grep 'magic-marker'; then echo 'Editable package already installed'; else pip install -e .; fi
...
release-major: clean-build develop lint test bump-major build publish ## package and upload a major release
	echo 'Successfully released!'
	echo 'Please push the newly created tag and commit to GitHub.'

release-minor: clean-build develop lint test bump-minor build publish ## package and upload a minor release
	echo 'Successfully released!'
	echo 'Please push the newly created tag and commit to GitHub.'

release-patch: clean-build develop lint test bump-patch build publish ## package and upload a patch release
	echo 'Successfully released!'
	echo 'Please push the newly created tag and commit to GitHub.'
